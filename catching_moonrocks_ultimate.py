...
def get_background_images(directory):
    """Returns list of background image paths"""
    try:
...
def load_background_for_level(level):
    """Load specific background for level"""
    bg_files = get_background_images(BACKGROUND_IMAGE_DIR)
    if bg_files and level <= len(bg_files):
        try:
            return Image.open(bg_files[level - 1])
        except:
            pass
    return None
...
def reset_level():
    ...
    # Load background
    st.session_state.background_image = load_background_for_level(st.session_state.level)
    bg_files = get_background_images(BACKGROUND_IMAGE_DIR)
